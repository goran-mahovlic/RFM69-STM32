...
#include <RFM69STM32.h>   // include the RFM69 library files as well
#include <RFM69registers.h>
#include <SPI.h>

volatile uint8_t RFM69_ATC_STM32::ACK_RSSI_REQUESTED;  // new type of flag on ACK_REQUEST

//=============================================================================
// initialize() - some extra initialization before calling base class
//=============================================================================
bool RFM69_ATC_STM32::initialize(uint8_t spiPort, uint8_t freqBand, uint8_t nodeID, uint8_t networkID) {
  ...
  return RFM69STM32::initialize(spiPort, freqBand, nodeID, networkID);  // use base class to initialize most everything
}

//=============================================================================
// setMode() - got to set updated transmit power level before switching to TX mode
//=============================================================================
void RFM69_ATC_STM32::setMode(uint8_t newMode) {
  if (newMode == _mode) return;
  //_powerBoost = (_transmitLevel >= 50);  // this needs to be set before changing mode just in case setHighPowerRegs is called
  RFM69STM32::setMode(newMode);  // call base class first
...
void RFM69_ATC_STM32::sendACK(const void* buffer, uint8_t bufferSize) {
...
void RFM69_ATC_STM32::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK) {
...
//=============================================================================
void RFM69_ATC_STM32::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK, bool sendRSSI, int16_t lastRSSI) {
  ...
  if (sendACK) {                   // TomWS1: adding logic to return ACK_RSSI if requested
    SPI_def.transfer(RFM69_CTL_SENDACK | (sendRSSI?RFM69_CTL_RESERVE1:0));  // TomWS1  TODO: Replace with EXT1
    if (sendRSSI) {
      SPI_def.transfer(abs(lastRSSI)); //RSSI dBm is negative expected between [-100 .. -20], convert to positive and pass along as single extra header byte
    ...
    SPI_def.transfer(_targetRSSI ? RFM69_CTL_REQACK | RFM69_CTL_RESERVE1 : RFM69_CTL_REQACK);
  }
  else SPI_def.transfer(0x00);
...
//=============================================================================
void RFM69_ATC_STM32::interruptHook(uint8_t CTLbyte) {
...
//=============================================================================
bool RFM69_ATC_STM32::sendWithRetry(uint8_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries, uint8_t retryWaitTime) {
  uint32_t sentTime;
...
//=============================================================================
void RFM69_ATC_STM32::receiveBegin() {
  ACK_RSSI_REQUESTED = 0;
  RFM69STM32::receiveBegin();
}
...
// void RFM69_ATC_STM32::setPowerLevel(uint8_t powerLevel) {
...
//=============================================================================
// void RFM69_ATC_STM32::setHighPower(bool onOff, byte PA_ctl) {
  // _isRFM69HW = onOff;
...
// ditto from above.
//=============================================================================
// void RFM69_ATC_STM32::setHighPowerRegs(bool onOff) {
...
void  RFM69_ATC_STM32::enableAutoPower(int16_t targetRSSI){    // TomWS1: New method to enable/disable auto Power control
...
//=============================================================================
int16_t  RFM69_ATC_STM32::getAckRSSI(void){                     // TomWS1: New method to retrieve the ack'd RSSI (if any)
...
//=============================================================================
byte RFM69_ATC_STM32::setLNA(byte newReg) {  // TomWS1: New method used to disable LNA AGC for testing purposes
  byte oldReg;
